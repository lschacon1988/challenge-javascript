...
function direcciones(laberintoExample) {
  let direccion = "";

  for (const valor in laberintoExample) {
    if (laberintoExample[valor] === "destino") return direccion + valor;
    if (laberintoExample[valor] !== "pared") {
      direccion = direccion + valor;

      return direccion + direcciones(laberintoExample[valor]);
    }
  }
  return direccion;
}
...
BinarySearchTree.prototype.toArray = function () {
  ...
  if (this.left) result.push(this.left.toArray());
  result.push(this.value);
  if (this.right) result.push(this.right.toArray());
